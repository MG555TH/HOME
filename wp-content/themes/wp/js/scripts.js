...
! function(e, t, r) {
    "use strict";
    var i = t.querySelectorAll(['iframe[src*="youtube.com"]', 'iframe[src*="vimeo.com"]'].join(","));
    if (i.length)
        for (var o = 0; o < i.length; o++) {
            var a = i[o],
                m = a.getAttribute("width"),
                n = a.getAttribute("height") / m,
                d = a.parentNode,
                c = t.createElement("div");
            c.className = "fitVids-wrapper", c.style.paddingBottom = 100 * n + "%", d.insertBefore(c, a), a.remove(), c.appendChild(a), a.removeAttribute("height"), a.removeAttribute("width")
        }
}(window, document);
...
const modalTriggers = document.querySelectorAll(".s-modal-trigger"),
    bodyBlackout = document.querySelector(".s-modal-bg"),
    allModals = document.querySelectorAll(".s-modal");
modalTriggers.forEach(e => {
    e.addEventListener("click", () => {
        const {
            popupTrigger: l
        } = e.dataset, o = document.querySelector(`[data-s-modal="${l}"]`);
        o.classList.add("-visible"), bodyBlackout.classList.add("-blacked-out"), o.querySelector(".s-modal-close").addEventListener("click", () => {
            o.classList.remove("-visible"), bodyBlackout.classList.remove("-blacked-out")
        })
    })
}), bodyBlackout.addEventListener("click", () => {
    bodyBlackout.classList.remove("-blacked-out"), allModals.forEach(function(e, l) {
